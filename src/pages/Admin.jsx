...
function downloadCSV(rows) {
  const headers = [
    'Team', 'Project', 'Members', 'GitHub', 'Email', 'Phone',
    'Submitted', 'Judges Scored', 'Total Score', 'Avg Score',
  ];
...
export default function Admin() {
  ...
  const stats = useMemo(() => {
    const fullyJudged = enriched.filter((t) => t.judges_scored >= totalJudges).length;
  ...
  return (
    <Layout status={`ADMIN · ${admin.username.toUpperCase()}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-10">
        ...
        <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Stat label="Submissions" value={stats.teams} />
          <Stat label="Judges active" value={stats.judges} />
          <Stat label="Fully judged" value={`${stats.fullyJudged}/${stats.teams}`} accent />
          <Stat label="Scores submitted" value={stats.totalScoresSubmitted} />
        </div>

        <div className="mt-3 font-mono text-[11px] uppercase tracking-[0.2em] text-ink/60">
          Last refresh · {updatedLabel}
        </div>
        ...
        <div className="mt-10">
          <h2 className="font-display font-black text-3xl mb-4">SUBMISSIONS</h2>
          {loading && teams.length === 0 ? (
            ...
            <div className="border-2 border-dashed border-ink/40 p-10 text-center">
              <div className="font-display italic font-black text-3xl">No submissions yet.</div>
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-ink text-paper">
                  <tr className="font-mono uppercase tracking-[0.18em] text-[11px]">
                    ...
                    <th className="p-3 hidden lg:table-cell w-32">REPO</th>
                    <th className="p-3 w-24 text-center">JUDGED</th>
                    <th className="p-3 w-24 text-right">TOTAL</th>
                    <th className="p-3 w-24 text-right">AVG</th>
                  </tr>
                </thead>
                <tbody>
                  {sorted.map((row, i) => {
                    ...
                    return (
                      <>
                        ...
                        >
                          <td className="p-3 text-center font-mono text-ink/40">
                            {isOpen ? '−' : '+'}
                          </td>
                          <td className="p-3">
                          ...
                          <td className="p-3 hidden md:table-cell text-ink/80">
                            {row.project_title}
                          </td>
                          ...
                          <td className="p-3 text-right font-mono font-bold tabular-nums text-xl">
                            {row.total_score}
                          </td>
                          ...
                          <tr className="bg-paper-dark border-t border-ink/15">
                            <td colSpan={7} className="p-0">
                              <div className="p-6 border-l-4 border-amber">
                                ...
                                <div className="mt-4 border-2 border-ink p-4 bg-paper">
                                  <div className="eyebrow mb-3">JUDGE BREAKDOWN</div>
                                  {judges.length === 0 ? (
                                    <p className="text-sm text-ink/60">No judges loaded.</p>
                                  ) : (
                                      ...
                                      <table className="w-full text-left text-sm">
                                        <thead>
                                          <tr className="font-mono uppercase tracking-wider text-[10px] text-ink/60">
                                            ...
                                            <th className="py-1 pr-3 text-right">INNOV</th>
                                            <th className="py-1 pr-3 text-right">EXEC</th>
                                            <th className="py-1 pr-3 text-right">PRES</th>
                                            <th className="py-1 pr-3 text-right">IMPACT</th>
