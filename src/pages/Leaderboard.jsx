...
function PodiumCard({ row, rank, size }) {
  const isOne = rank === 1;
  const rankSize = {
    ...
    >
      <div className="flex items-start justify-between">
        ...
        <span className="font-mono text-[11px] uppercase tracking-wider text-ink/60">
          {row.judges_scored}/{REQUIRED_JUDGES} judges
        </span>
      </div>
      <div
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-12">
        <div className="flex flex-wrap items-end justify-between gap-6">
          ...
          <div className="flex items-center gap-3 border-2 border-ink bg-paper px-4 py-2 shadow-brut-sm">
            ...
            <span className="font-mono text-[11px] tabular-nums text-ink/60 hidden sm:inline">
              · {updatedLabel}
            </span>
          </div>
        </div>
        ...
        <section className="mt-12">
          {loading && rows.length === 0 ? (
            ...
            <div className="border-2 border-dashed border-ink/40 p-10 text-center">
              <div className="font-display italic font-black text-3xl">
                Awaiting full ballots.
              </div>
              <p className="mt-2 text-ink/60 text-sm">
                The podium fills in once every judge has scored a team.
              </p>
            </div>
          ) : (
            <div className="grid lg:grid-cols-3 gap-6 items-end">
              {podium[1] ? <PodiumCard row={podium[1]} rank={2} /> : <div className="hidden lg:block" />}
              {podium[0] ? <PodiumCard row={podium[0]} rank={1} /> : null}
              {podium[2] ? <PodiumCard row={podium[2]} rank={3} /> : <div className="hidden lg:block" />}
            </div>
          )}
        </section>
        ...
        <section className="mt-16">
          <div className="flex items-baseline justify-between flex-wrap gap-2 mb-4">
            <h2 className="font-display font-black text-3xl">FULL RANKING</h2>
            <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-ink/60">
              Pending = waiting on all {REQUIRED_JUDGES} judges
            </span>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-ink text-paper">
                <tr className="font-mono uppercase tracking-[0.18em] text-[11px]">
                  ...
                  <th className="p-3 hidden sm:table-cell">PROJECT</th>
                  <th className="p-3 w-24 text-center">JUDGES</th>
              ...
              <tbody>
                ...
                {complete.map((row, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3 hidden sm:table-cell text-ink/80">
                        {row.project_title}
                      </td>
                ...
                {pending.map((row, i) => (
                  <tr
                    key={row.team_id}
                    className={`border-t border-ink/15 ${
                      (complete.length + i) % 2 === 0 ? 'bg-paper' : 'bg-paper-dark'
                    } text-ink/70`}
                  >
